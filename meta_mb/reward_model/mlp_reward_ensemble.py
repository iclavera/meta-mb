...
class MLPDynamicsEnsemble(MLPRewardModel):
    """
    ...
    def __init__(self,
        ...
        with tf.variable_scope(name, reuse=True):
            ...
            self.nextobs_model_batches_stack_ph = tf.placeholder(tf.float32, shape=(None, obs_space_dims))
            ...
            for i in range(num_models):
                with tf.variable_scope('model_{}'.format(i), reuse=True):
                    # concatenate action and observation --> NN input
                    nn_input = tf.concat(
                        [self.obs_model_batches[i], self.act_model_batches[i], self.reward_model_batches[i]], axis=1)
                    mlp = MLP(name + '/model_{}'.format(i),
                              output_dim=1,
                              hidden_sizes=hidden_sizes,
                              hidden_nonlinearity=hidden_nonlinearity,
                ...
                reward_preds.append(mlp.output_var)
                if loss_str == 'L2':
                    loss = tf.reduce_mean(tf.linalg.norm(self.reward_ph[:, :, None] - self.reward_pred, axis=1))
                elif loss_str == 'MSE':
                    loss = tf.reduce_mean((self.reward_ph[:, :, None] - self.reward_pred) ** 2)
                else:
                    raise NotImplementedError
    ...
    def update_buffer(self, obs, act, obs_next, reward, valid_split_ratio=None, check_init=True):
        ...
        obs_train_batches = []
        act_train_batches = []
        nextobs_train_batches = []
        reward_train_batches = []
        obs_test_batches = []
        act_test_batches = []
        nextobs_test_batches = []
        reward_test_batches = []
        ...
        for i in range(self.num_models):
            ...
            nextobs_train_batches.append(nextobs_train)
            reward_train_batches.append(reward_train)
            obs_test_batches.append(obs_test)
            act_test_batches.append(act_test)
            nextobs_test_batches.append(nextobs_test)
            reward_test_batches.append(reward_test)
            # create data queue

        # If case should be entered exactly once
        if check_init and self._dataset_test is None:
            self._dataset_test = dict(obs=obs_test_batches, act=act_test_batches, nextobs=nextobs_test_batches,
                                      reward=reward_test_batches)
            self._dataset_train = dict(obs=obs_train_batches, act=act_train_batches, nextobs=nextobs_train_batches,
                                       reward=reward_train_batches)
            ...
            for i in range(self.num_models):
                ...
                self._dataset_test['act'][i] = np.concatenate([self._dataset_test['act'][i][-n_max_test:],
                                                               act_test_batches[i]])
                self._dataset_test['nextobs'][i] = np.concatenate([self._dataset_test['nextobs'][i][-n_max_test:],
                                                                   nextobs_test_batches[i]])
                self._dataset_test['reward'][i] = np.concatenate([self._dataset_test['reward'][i][-n_max_test:],
                                                                  reward_test_batches[i]])
                ...
                self._dataset_train['act'][i] = np.concatenate([self._dataset_train['act'][i][-n_max_train:],
                                                                act_train_batches[i]])
                self._dataset_train['nextobs'][i] = np.concatenate(
                    [self._dataset_train['nextobs'][i][-n_max_train:], nextobs_train_batches[i]])
                self._dataset_train['reward'][i] = np.concatenate(
                    [self._dataset_train['reward'][i][-n_max_train:], reward_train_batches[i]])
    ...
    def fit_one_epoch(self, remaining_model_idx, valid_loss_rolling_average_prev, with_new_data,
        ...
        if self.normalize_input:
            ...
            obs_train, act_train, nextobs_train, reward_train = self._dataset_train['obs'], self._dataset_train['act'], \
                                                                self._dataset_train['nextobs'], self._dataset_train[
                                                                    'reward']
        ...
        feed_dict = dict(
            ...
            list(zip(self.nextobs_batches_dataset_ph, nextobs_train)) +
            list(zip(self.reward_batches_dataset_ph, reward_train))
        )
        ...
        while True:
            try:
                obs_act_nextobs_rew = sess.run(self.next_batch)
                obs_batch_stack = np.concatenate(obs_act_nextobs_rew[:self.num_models], axis=0)
                act_batch_stack = np.concatenate(obs_act_nextobs_rew[self.num_models:2 * self.num_models], axis=0)
                nextobs_batch_stack = np.concatenate(obs_act_nextobs_rew[2 * self.num_models:3 * self.num_models],
                                                     axis=0)
                reward_batch_stack = np.concatenate(obs_act_nextobs_rew[3 * self.num_models:], axis=0)
                ...
                if self.normalize_input:
                    ...
                    obs_test, act_test, nextobs_test, reward_test = self._normalize_data(self._dataset_test['obs'],
                                                                                         ...
                                                                                         self._dataset_test['nextobs'],
                                                                                         self._dataset_test['reward'])

                else:
                    obs_test, act_test, nextobs_test, reward_test = self._dataset_test['obs'], self._dataset_test[
                        'act'], \
                                                                    self._dataset_test['nextobs'], self._dataset_test[
                                                                        'reward']

                obs_test_stack = np.concatenate(obs_test, axis=0)
                act_test_stack = np.concatenate(act_test, axis=0)
                nextobs_test_stack = np.concatenate(nextobs_test, axis=0)
                reward_test_stack = np.concatenate(reward_test, axis=0)
    ...
    def fit(self, obs, act, obs_next, epochs=1000, compute_normalization=True,
        ...
        for epoch in range(epochs):
            ...
            while True:
                try:
                    obs_act_nextobs_reward = sess.run(self.next_batch)
                    obs_batch_stack = np.concatenate(obs_act_nextobs_reward[:self.num_models], axis=0)
                    act_batch_stack = np.concatenate(obs_act_nextobs_reward[self.num_models:2 * self.num_models],
                                                     axis=0)
                    nextobs_batch_stack = np.concatenate(
                        obs_act_nextobs_reward[2 * self.num_models:3 * self.num_models], axis=0)
                    reward_batch_stack = np.concatenate(obs_act_nextobs_reward[3 * self.num_models:4 * self.num_models],
                                                        axis=0)
                    ...
                    if self.normalize_input:
                        ...
                        obs_test, act_test, nextobs_test, reward_test = self._dataset_test['obs'], self._dataset_test[
                            'act'],
                        self._dataset_test['nextobs'], self._dataset_test['reward']
        ...
        if not idx_to_remove: epoch_times.append(
            time.time() - epoch_start_time)  # only track epoch times while all models are trained
    ...
    with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
        for i in range(self.num_models):
            with tf.variable_scope('model_{}'.format(i), reuse=True):
                ...
                in_nextobs_var = (nextobs_ph[i] - self._mean_nextobs_var[i]) / (self._std_nextobs_var[i] + 1e-8)
                ...
                reward_pred = mlp.output_var * self._std_reward_var[i] + self._mean_reward_var[i]
...
def predict_batches_sym(self, obs_ph, act_ph, nextobs_ph):
    """
    Same batch fed into all models. Randomly output one of the predictions for each observation.
    :param obs_ph: (batch_size, obs_space_dims)
    :param act_ph: (batch_size, act_space_dims)
    ...
    # unshuffle
    reward = tf.clip_by_value(reward_preds, -1e2, 1e2)
    return reward


def predict(self, obs, act, nextobs, pred_type='rand', **kwargs):
    """
    ...
    if self.normalize_input:
        obs, act, nextobs = [obs for _ in range(self.num_models)], [act for _ in range(self.num_models)], \
                            [nextobs for _ in range(self.num_models)]
        obs, act, nextobs = self._normalize_data(obs, act, nextobs)
        obs, act, nextobs = np.concatenate(obs, axis=0), np.concatenate(act, axis=0), np.concatenate(nextobs, axis=0)
        reward = np.array(self.f_reward_pred(obs, act, nextobs))
        reward = self._denormalize_data(reward)
    ...
    if pred_type == 'rand':
        ...
        pred_obs = np.stack([reward[row, :, model_id] for row, model_id in enumerate(idx)], axis=0)
    elif pred_type == 'mean':
        pred_obs = np.mean(reward, axis=2)
    elif pred_type == 'all':
        pass
    else:
        NotImplementedError('pred_type must be one of [rand, mean, all]')

    pred_obs = np.clip(reward, -1e2, 1e2)
    return reward


def predict_batches(self, obs_batches, act_batches, nextobs_batches, *args, **kwargs):
    """
    ...
    if self.normalize_input:
        # Normalize Input
        obs_batches, act_batches, nextobs_batches = np.split(obs_batches, self.num_models), \
                                                    np.split(act_batches, self.num_models), \
                                                    np.split(nextobs_batches, self.num_models)
        obs_batches, act_batches, nextobs_batches = self._normalize_data(obs_batches, act_batches, nextobs_batches)
        obs_batches, act_batches, nextobs_batches = np.concatenate(obs_batches, axis=0), \
                                                    np.concatenate(act_batches, axis=0), \
                                                    np.concatenate(nextobs_batches, axis=0)

        reward_batches = np.array(self.f_reward_pred_model_batches(obs_batches, act_batches, nextobs_batches))

        # Denormalize output
        reward_batches = np.array(np.split(reward_batches, self.num_models)).transpose((1, 2, 0))
        reward_batches = self._denormalize_data(reward_batches)
        reward_batches = np.concatenate(reward_batches.transpose((2, 0, 1)), axis=0)

    else:
        reward_batches = np.array(self.f_reward_pred(obs_batches, act_batches, nextobs_batches))

    assert reward_batches.ndim == 2

    reward_batches = np.clip(reward_batches, -1e2, 1e2)
    assert reward_batches.shape == obs_batches.shape
...
def _data_input_fn(self, obs_batches, act_batches, nextobs_batches, reward_batches, batch_size=500, buffer_size=5000):
    """ Takes in train data an creates an a symbolic nex_batch operator as well as an iterator object """

    assert len(obs_batches) == len(act_batches) == len(nextobs_batches) == len(reward_batches)
    obs, act, nextobs, reward = obs_batches[0], act_batches[0], nextobs_batches[0], reward_batches[0]
    ...
    self.nextobs_batches_dataset_ph = [tf.placeholder(tf.float32, (None, nextobs.shape[1])) for _ in
                                       range(self.num_models)]
    self.reward_batches_dataset_ph = [tf.placeholder(tf.float32, (None, reward.shape[1])) for _ in
                                      range(self.num_models)]

    dataset = tf.data.Dataset.from_tensor_slices(
        tuple(
            self.obs_batches_dataset_ph + self.act_batches_dataset_ph + self.nextobs_batches_dataset_ph + self.reward_batches_dataset_ph)
    )
...
def compute_normalization(self, obs, act, nextobs, reward):
    assert len(obs) == len(act) == len(nextobs) == len(reward) == self.num_models
    assert all(
        [o.shape[0] == r.shape[0] == a.shape[0] == nexto.shape[0] for o, a, nexto, r in zip(obs, act, nextobs, reward)])
    assert all([nexto.shape[1] == o.shape[1] for nexto, o in zip(obs, nextobs)])
    ...
    for i in range(self.num_models):
        ...
        normalization['nextobs'] = (np.mean(nextobs[i], axis=0), np.std(nextobs[i], axis=0))
...
def _create_stats_vars(self):
    ...
    self._mean_nextobs_var, self._std_nextobs_var, self._mean_nextobs_ph, self._std_nextobs_ph = [], [], [], []
    ...
    for i in range(self.num_models):
        ...
        self._std_act_var.append(tf.get_variable('std_act_%d' % i, shape=(self.action_space_dims,),
                                                 dtype=tf.float32, initializer=tf.ones_initializer, trainable=False))
        self._mean_nextobs_var.append(tf.get_variable('mean_nextobs_%d' % i, shape=(self.obs_space_dims,),
                                                      dtype=tf.float32, initializer=tf.zeros_initializer,
                                                      trainable=False))
        self._std_nextobs_var.append(tf.get_variable('std_nextobs_%d' % i, shape=(self.obs_space_dims,),
                                                     dtype=tf.float32, initializer=tf.ones_initializer,
                                                     trainable=False))
        self._mean_reward_var.append(tf.get_variable('mean_reward_%d' % i, shape=(1,),
                                                     dtype=tf.float32, initializer=tf.zeros_initializer,
                                                     trainable=False))
        self._std_reward_var.append(tf.get_variable('std_reward_%d' % i, shape=(1,),
                                                    dtype=tf.float32, initializer=tf.ones_initializer, trainable=False))
        ...
        self._mean_nextobs_ph.append(tf.placeholder(tf.float32, shape=(self.obs_space_dims,)))
        self._std_nextobs_ph.append(tf.placeholder(tf.float32, shape=(self.obs_space_dims,)))
        self._mean_reward_ph.append(tf.placeholder(tf.float32, shape=(1,)))
        self._std_reward_ph.append(tf.placeholder(tf.float32, shape=(1,)))
        ...
        self._assignations.extend([tf.assign(self._mean_obs_var[i], self._mean_obs_ph[i]),
                                   ...
                                   tf.assign(self._mean_nextobs_var[i], self._mean_nextobs_ph[i]),
                                   tf.assign(self._std_nextobs_var[i], self._std_nextobs_ph[i]),
...
def _normalize_data(self, obs, act, nextobs, reward=None):
    ...
    for i in range(self.num_models):
        ...
        norm_nextobs = normalize(nextobs[i], self.normalization[i]['nextobs'][0], self.normalization[i]['nextobs'][1])
        ...
        norm_nextobses.append(norm_nextobs)
        if reward is not None:
            assert len(reward) == self.num_models
    ...
    if reward is not None:
        return norm_obses, norm_acts, norm_nextobses, norm_rewards

    return norm_obses, norm_acts, norm_nextobses


def _denormalize_data(self, reward):
    assert reward.shape[-1] == self.num_models
    denorm_rewards = []
    for i in range(self.num_models):
        denorm_reward = denormalize(reward[..., i], self.normalization[i]['reward'][0],
                                    self.normalization[i]['reward'][1])
        denorm_rewards.append(denorm_reward)
    return np.stack(denorm_rewards, axis=-1)
