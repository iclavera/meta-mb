...
class MLPRewardEnsemble(MLPRewardModel):
    ...
    def update_buffer(self, obs, act, obs_next, reward, valid_split_ratio=None, check_init=True):
        ...
        for i in range(self.num_models):
            obs_train, act_train, nextobs_train, reward_train, obs_test, act_test, nextobs_test, reward_test = \
                train_test_split(obs, act, obs_next, reward=reward.reshape((-1, 1)), test_split_ratio=valid_split_ratio)
    ...
    def fit(self, obs, act, obs_next, reward, epochs=1000, compute_normalization=True,
        ...
        if obs is not None:
            self.update_buffer(obs, act, obs_next, reward.reshape((-1, 1)), valid_split_ratio, compute_normalization)
            ...
            while True:
                try:
                    ...
                    if self.normalize_input:
                        ...
                        obs_test, act_test, nextobs_test, reward_test = self._dataset_test['obs'], self._dataset_test['act'],\
    ...
    def predict(self, obs, act, nextobs, pred_type='rand', **kwargs):
        ...
        if pred_type == 'rand':
            ...
            reward = np.stack([reward[row, :, model_id] for row, model_id in enumerate(idx)], axis=0)
        elif pred_type == 'mean':
            reward = np.mean(reward, axis=2)
        elif pred_type == 'all':
            pass
        else:
    ...
    def predict_batches(self, obs_batches, act_batches, nextobs_batches, *args, **kwargs):
        ...
        return reward_batches.reshape((-1, ))
