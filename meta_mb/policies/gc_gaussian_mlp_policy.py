...
class GCGaussianMLPPolicy(Policy):
    """
    ...
    def build_graph(self):
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            self.input_ph, self.output_var = create_mlp(name='network',
            ...
            self.log_std_var = tf.clip_by_value(self.log_std_var, self.min_log_std, self.max_log_std, name='log_std')
    ...
    def get_action(self, observation, goal):
        ...
        action, agent_infos = self.get_actions(observation, goal)
    ...
    def get_actions(self, observations, goals):
        ...
        assert goals.ndim == 2 and goals.shape[1] == self.goal_dim

        sess = tf.get_default_session()
        actions, means, log_stds = sess.run([self.action_var, self.mean_var, self.log_std_var],
                                             feed_dict={self.input_ph: np.concatenate([observations, goals], axis=1)})
        if self.squashed:
    ...
    def distribution_info_sym(self, input_var, params=None):
        """
        ...
        if params is None:
            with tf.variable_scope(self.name, reuse=True):
                input_var, output_var = create_mlp(name='network',
                                               output_dim=2 * self.action_dim,
                                               hidden_sizes=self.hidden_sizes,
                                               hidden_nonlinearity=self.hidden_nonlinearity,
                                               output_nonlinearity=self.output_nonlinearity,
                ...
                self.log_std_var = tf.clip_by_value(self.log_std_var, self.min_log_std, self.max_log_std,
                                                    name='log_std')
        else:
            input_var, output_var = forward_mlp(output_dim=2 * self.action_dim,
                                            hidden_sizes=self.hidden_sizes,
                                            hidden_nonlinearity=self.hidden_nonlinearity,
                                            output_nonlinearity=self.output_nonlinearity,
                                            input_var=input_var,
