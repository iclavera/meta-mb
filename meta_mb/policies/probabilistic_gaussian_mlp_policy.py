...
class ProbGaussianMLPPolicy(Policy):
    """
    ...
    def build_graph(self):
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            self.obs_var, mean_log_std = create_mlp(name='mean_log_std_network',
            ...
            self.mean_var, log_std_var = tf.split(mean_log_std, 2, axis=-1)
            self.log_std_var = tf.maximum(log_std_var, self.min_log_std)
            self.action_var = self.mean_var + tf.multiply(tf.random_normal(shape=tf.shape(self.mean_var)), tf.exp(self.log_std_var))
            ...
            self.policy_params_ph = self._create_placeholders_for_vars(scope=self.name + "/mean_log_std_network")
    ...
    def distribution_info_sym(self, obs_var, params=None):
        ...
        if params is None:
            with tf.variable_scope(self.name):
                obs_var, mean_log_std = create_mlp(name='mean_log_std_network',
                                               output_dim=2*self.action_dim,
                                               hidden_sizes=self.hidden_sizes,
                                               hidden_nonlinearity=self.hidden_nonlinearity,
                                               output_nonlinearity=self.output_nonlinearity,
                ...
                mean_var, log_std_var = tf.split(mean_log_std, 2, axis=-1)
                log_std_var = tf.maximum(log_std_var, self.min_log_std)
        else:
            obs_var, mean_log_std = forward_mlp(output_dim=2*self.action_dim,
                                            hidden_sizes=self.hidden_sizes,
                                            hidden_nonlinearity=self.hidden_nonlinearity,
                                            output_nonlinearity=self.output_nonlinearity,
                                            input_var=obs_var,
