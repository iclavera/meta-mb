...
class MPCController(Serializable):
    ...
    def get_cem_action(self, observations):
        ...
        mean = tf.ones(shape=[self.horizon, tf.shape(observations)[0], 1,
                              self.action_space_dims]) * (
                           self.env.action_space.high + self.env.action_space.low) / 2
        var = tf.ones(shape=[self.horizon, tf.shape(observations)[0], 1,
                             self.action_space_dims]) * (self.env.action_space.high - self.env.action_space.low) / 2
        ...
        for itr in range(self.num_cem_iters):
            ...
            constrained_var = var
            std = tf.sqrt(constrained_var + 1e-6)
            act = mean + tf.random.normal(shape=[self.horizon, tf.shape(observations)[0], self.n_candidates,
                                                 self.action_space_dims]) * std
            act = tf.clip_by_value(act, self.env.action_space.low, self.env.action_space.high)
            returns = 0
            observation = tf.reshape(
                tf.tile(tf.expand_dims(observations, -1), [1, self.n_candidates, 1]),
                [-1, self.obs_space_dims]
            )
            ...
            for t in range(self.horizon):
                ...
                if not self.use_reward_model:
                    ...
                    assert not (self.reward_model is None)
                    rewards = self.reward_model.predict(observation, act[t], next_observation)
                returns += self.discount ** t * rewards
                observation = next_observation

            # Re-fit belief to the best ones.
            returns = tf.reshape(returns,
                                 (tf.shape(observations)[0], self.n_candidates))  # (batch_size, n_candidates)
            act = tf.reshape(act, shape=[self.horizon, tf.shape(observations)[0], self.n_candidates,
                                         self.action_space_dims])
            _, indices = tf.nn.top_k(returns, self.num_elites, sorted=False)
            act = tf.transpose(act, (1, 2, 3, 0))  # (batch_size, n_candidates, obs_dim, horizon)
            elite_actions = tf.batch_gather(act, indices)
            elite_actions = tf.transpose(elite_actions,
                                         (3, 0, 1, 2))  # (horizon, batch_size, n_candidates, obs_dim)
            elite_mean, elite_var = tf.nn.moments(elite_actions, axes=[2])
            elite_mean, elite_var = tf.expand_dims(elite_mean, axis=2), tf.expand_dims(elite_var, axis=2)
            mean = mean * self.alpha + (1 - self.alpha) * elite_mean
            var = var * self.alpha + (1 - self.alpha) * elite_var

        return tf.get_default_session().run(tf.squeeze(mean[0], axis=1))
    ...
    #     mean = np.ones((m, h * act_dim)) * np.concatenate([(self.env.action_space.high + self.env.action_space.low) / 2] * h)
    #     std = np.ones((m, h * act_dim)) * np.concatenate([(self.env.action_space.high - self.env.action_space.low) / 16] * h)
    ...
    #         # TODO: this is for the ground truth model only!
    #         # import pdb; pdb.set_trace()
    #         # if hasattr(self.dynamics_model, 'reset_from_obs'):
    #         #     self.dynamics_model.reset_from_obs(observation)
    #         for t in range(h):
    ...
    #         lb_dist, ub_dist = mean - clip_low, clip_high - mean
