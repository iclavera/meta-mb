...
class ImgWrapperEnv(Serializable):
    ...
    def step(self, action):
        ...
        if self.encoder is not None:
            obs = self.encoder.encode(self._obs).reshape((self.latent_dim,))
    ...
    def observation_space(self):
        if self.latent_dim is not None:
            assert self.use_img
            return Box(-1e6 * np.ones((self.latent_dim,)),
                       1e6 * np.ones((self.latent_dim,)), dtype=np.float32)

        return Box(-1e6 * np.ones(self._img_size),
                   1e6 * np.ones(self._img_size),
                   dtype=np.float32)
