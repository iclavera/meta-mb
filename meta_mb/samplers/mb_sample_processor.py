...
class ModelSampleProcessor(SampleProcessor):
    ...
    def _concatenate_path_data(self, paths):
        ...
        next_observations = np.concatenate([np.concatenate([path["observations"][1:], path["observations"][-1][None]])
                                                   for path in paths])
        actions = np.concatenate([path["actions"] for path in paths])
        rewards = np.concatenate([path["rewards"] for path in paths])
        dones = np.concatenate([path["dones"] for path in paths])
        returns = np.concatenate([path["returns"] for path in paths])
        time_steps = np.concatenate([np.arange(len(path["observations"])) for path in paths])
