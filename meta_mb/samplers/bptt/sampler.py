...
class BPTTSampler(BaseSampler):
    ...
    def __init__(
        ...
        self._initial_obs_ph = tf.placeholder(
            shape=(self.num_envs, self.policy.obs_dim),
            dtype=tf.float32,
            name='init_obs',
        )
        self.build_graph()
    ...
    def build_graph(self):
        ...
        for t in range(self.max_path_length):
            act = self.policy.get_actions_sym(obs)
    ...
    def obtain_samples(self, log=False, log_prefix='', buffer=None, random=False):
        ...
        init_obses = np.array([self.env.reset() for _ in range(self.num_envs)])
        ...
        if log_stds.shape[0] == 1:
            log_stds = np.repeat(log_stds, self.num_envs, axis=0)
        ...
        dones = [[False for _ in range(self.max_path_length)] for _ in range(self.num_envs)]
    ...
    def optimize_policy(self, log=True):
        init_obses = np.array([self.env.reset() for _ in range(self.num_envs)] * self.num_models)
