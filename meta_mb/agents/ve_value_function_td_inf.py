...
class ValueFunction(Serializable):
    ...
    def _build_placeholder(self):
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            self.return_ph = tf.placeholder(dtype=tf.float32, shape=(None,), name='rewards')

            self.op_phs_dict = dict(observations=self.obs_ph, returns=self.return_ph,
                                    goals=self.goal_ph)
    ...
    def _init_td_inf_update(self):
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            return_var = tf.expand_dims(self.return_ph, axis=1)

            loss = tf.losses.mean_squared_error(labels=return_var, predictions=v_predict)
