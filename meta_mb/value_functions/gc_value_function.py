...
class ValueFunction(Serializable):
    ...
    def build_graph(self):
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            self.input_var, self.output_var = create_mlp(name='v_network',
                                                     ...
                                                     input_dim=(None, self.obs_dim + self.action_dim + self.goal_dim,),
                                                     )
    ...
    def compute_values(self, obs, actions, goals):
        return self._vfun_np(np.concatenate([obs, actions, goals], axis=1)).flatten()
