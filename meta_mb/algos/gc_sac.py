...
class SAC(Algo):
    ...
    def _get_q_target(self):
        goal_ph = self.op_phs_dict['goals']
        next_observations_ph = self.op_phs_dict['next_observations']
        dist_info_sym = self.policy.distribution_info_sym(tf.concat([next_observations_ph, goal_ph], axis=-1))
        ...
        input_q_fun = tf.concat([next_observations_ph, next_actions_var, goal_ph], axis=-1)
    ...
    def _init_critic_update(self):
        ...
        goal_ph = self.op_phs_dict['goals']
        input_q_fun = tf.concat([observations_ph, actions_ph, goal_ph], axis=-1)
    ...
    def _init_actor_update(self):
        ...
        observations_ph = self.op_phs_dict['observations']
        goal_ph = self.op_phs_dict['goals']
        dist_info_sym = self.policy.distribution_info_sym(tf.concat([observations_ph, goal_ph], axis=-1))
        ...
        input_q_fun = tf.concat([observations_ph, actions_var, goal_ph], axis=-1)
