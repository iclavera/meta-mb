...
class Trainer(object):
    ...
    def eval(self):
        ...
        with self.sess.as_default() as sess:
            ...
            logger.log("Start to evaluate without MPC")
            ...
            _ = self.eval_env_sample_processor.process_samples(multiple_trajectories, log='all', log_prefix='no-mpc-')

            logger.log("Start to evaluate with MPC")
            ...
            multiple_trajectories = self.eval_env_sampler.obtain_samples(log=True,
                                                                         ...
                                                                         multiple_trajectory = self.num_eval_trajectories,
                                                                         dynamics_model = self.dynamics_model)
            _ = self.eval_env_sample_processor.process_samples(multiple_trajectories, log='all', log_prefix='mpc-')
    ...
    def train(self):
        with self.sess.as_default() as sess:
            ...
            add_data_num = (self.start_itr-1)*1000 + self.n_initial_exploration_steps
            while self.env_replay_buffer._size < add_data_num:
            ...
            for _ in range(4*(self.start_itr-1)):
            ...
            time_step = (self.start_itr - 1) * 1000 * self.n_train_repeats
            self.algo._update_target(tau=1.0)
            for _ in range(1000* self.start_itr-1):
                self.algo._update_target()
                self.itr = itr
