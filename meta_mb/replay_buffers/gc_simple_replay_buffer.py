...
class SimpleReplayBuffer(ReplayBuffer, Serializable):
    ...
    def add_sample(self, goal, observation, action, reward, terminal, next_observation, **kwargs):
    ...
    def add_samples(self, goals, observations, actions, rewards, terminals, next_observations, **kwargs):
        ...
        if self._top + total_num <= self._max_buffer_size:
            ...
            if redundant == 0:
                self._goals[self._top:] = goals[:back_size]
                ...
                self._goals[:total_num - back_size] = goals[back_size:]
                ...
                self._goals[:remaining] = goals[back_size + redundant * self._max_buffer_size:]
    ...
    def __setstate__(self, d):
        ...
        self._goals = np.fromstring(d['g']).reshape(
