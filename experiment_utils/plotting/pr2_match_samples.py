...
data_path = './data/corl_data/pr2_data/match/'
    ...
    for exp in exp_data:
        if not asynch:
            ...
            if sup_y_key is not None:
                ...
                for key in sup_y_key:
                    if key in exp['progress'].keys():
                        if 'Data-TotalStep' in exp['progress'].keys():
                            x_y_tuples.extend(list(zip(exp['progress']['Data-TotalStep'], exp['progress'][key]/3 * 100)))
                        break
            else:
...
plot_from_exps(exps_data,
               ...
               plot_name='./pr2_match_samples.pdf',
               num_rows=1,
               report_max_performance=False,
               log_scale=False,
               round_x=2,
