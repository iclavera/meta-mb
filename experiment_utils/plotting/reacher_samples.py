...
data_path = './data/corl_data/video_reaching/'
...
x_limits = {'PR2Reach': [0, 1500],
            'meta_mb.envs.pr2.real_pr2_reach_env.PR2ReachEnv': [0, 1500],
            }
    ...
    for exp in exp_data:
        if not asynch:
            ...
            if sup_y_key is not None:
                ...
                for key in sup_y_key:
                    if key in exp['progress'].keys():
                        x_y_tuples.extend(list(zip(exp['progress']['Data-TotalStep'], exp['progress'][key])))
                        break
            else:
...
def plot_from_exps(exp_data,
    ...
    fig, axarr = plt.subplots(num_rows, num_columns, figsize=(16, 16))
    axarr = np.reshape(axarr, (num_rows, num_columns))
    fig.tight_layout(pad=8.0, w_pad=6, h_pad=4, rect=[0, 0, 1, 1])
...
plot_from_exps(exps_data,
               ...
               plot_name='./pr2_video_reaching_samples.png',
               num_rows=1,
               report_max_performance=False,
               log_scale=False,
               round_x=5,
