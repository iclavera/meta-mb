...
from meta_mb.envs.point_envs.point_env_2d import MetaPointEnv
...
if __name__=="__main__":
    ...
    logger.configure(dir=maml_zoo_path + '/data/trpo/test_%d' % idx, format_strs=['stdout', 'log', 'csv'],
                     snapshot_mode='last_gap')
    config = json.load(open(maml_zoo_path + "/configs/trpo_maml_config.json", 'r'))
    json.dump(config, open(maml_zoo_path + '/data/trpo/test_%d/params.json' % idx, 'w'))
    main(config)
