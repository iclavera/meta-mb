...
def run_experiment(**config):
    ...
    if config['encoder'] == 'cpc':
        ...
        folder =  'vae-%s-code%d-%s' % (config['env'], config['latent_dim'], config['run_suffix'])
    ...
    with sess.as_default() as sess:
        if config['use_image']:
            if config['encoder'] == 'cpc':
                ...
                env = ImgWrapperEnv(NormalizedEnv(raw_env), time_steps=1, vae=encoder, latent_dim=config['latent_dim'])
        else:
